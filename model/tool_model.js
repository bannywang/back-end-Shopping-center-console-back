    ...
    `
        ...
        results.forEach((result) => {
            if (result.id !== currentOrderId) {
                ...
                const { id, create_time, user_id, name, recipient_name, recipient_phone, recipient_address, sum } =
                    result
                orders.push({
                    id,
                    time: create_time,
...
async function vague_search_order_only_cancel_or_finish(order_number_or_order_name) {
    if (!order_number_or_order_name || order_number_or_order_name.trim() === '') {
        console.log('請提供有效的訂單編號或訂單姓名')
        return false
    }
...
async function get_administrator_grade(administrator_id) {
    try {
        ...
        const query_grade = `
            ...
            WHERE id = ?
        `
        const [results_grade] = await connection.query(query_grade, [administrator_id])
...
async function check_administrator_existence(administrator_id) {
    ...
    `
        ...
        const [administrator_results] = await connection.query(check_administrator_query, check_administrator_values)
...
async function get_store_revenue() {
    try {
        const query = 'SELECT IFNULL(SUM(sum), 0) AS total_revenue FROM purchase_history WHERE purchase_status = 4'
        const [results] = await connection.query(query)
...
async function get_store_interval_revenue(start_date, end_date) {
    try {
        ...
        const total_revenue = results[0].total_revenue
        console.log(`${start_date} 至 ${end_date} 期間，總收入為 ${total_revenue} 元`)
        return total_revenue
    } catch (error) {
        console.error('取得收入資料失敗：', error)
        return 0
...
function createExcelReport(data) {
    ...
    // 設定每個欄的寬度
    const columnWidths = [10, 20, 10, 15, 20, 15, 30, 15, 25, 10, 10, 8, 12, 12, 12] // 增加了一個欄位，因此調整了寬度

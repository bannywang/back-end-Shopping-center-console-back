...
async function add_new_QA(req, res) {
    ...
    try {
        if (id_card) {
            ...
            const new_QA_data = await service.create_customer_service_QA(question, answer)
            console.log('新的ＱＡ！')
...
async function update_QA(req, res) {
    ...
    try {
        if (id_card) {
            ...
            const question = req.body.question
            const answer = req.body.answer
            const update_QA_data = await service.use_id_update_customer_service_QA(QA_id, question, answer)
            console.log('異動ＱＡ！')
